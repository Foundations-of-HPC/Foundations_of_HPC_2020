	  ...
	  for (int i = 0; i < N; i++)
	    {
	      ...
	      for(j = 0; j < N4; j+=4)
		{
		  array_swap[offset + j+1] = array[j*(N+1) + i];
		  array_swap[offset + j+2] = array[(j+1)*(N+1) + i];
		  array_swap[offset + j+3] = array[(j+2)*(N+1) + i];
		  array_swap[offset + j+4] = array[(j+3)*(N+1) + i];
		}
	      for(; j < N; j++)
		array_swap[offset + j] = array[j*(N+1) + i];
	    }
	}
